...
function systemPrompt(net: boolean, w: number, h: number): string {
  return [
    ...
    '  "wire a table into this widget to begin" — never invent rows to fill space.',
    '',
    ...
    '- tableId and rowId MUST come from plexi.getInputs(). A table that is not',
    '  wired in is refused — this widget can only change what it can see.',
